...
class Pendulos extends THREE.Object3D {
   constructor(gui, titleGui) {
      ...
      this.tiempoAnterior2 = Date.now();

      // Y añadirlo como hijo del Object3D (el this)
      this.add(this.pendulo1);
      this.tiempoAnterior1 = Date.now();
   }
   ...
   createGUI(gui, titleGui) {
      ...
      var animacion = gui.addFolder("Animación");
      ...
      animacion.add(this.guiControls, 'animacion1').name("Péndulo 1 : ").listen();
      animacion.add(this.guiControls, 'velocidad1', 0.0, 2.0, 0.1).name("Velocidad (rad/s) : ").listen();
      animacion.add(this.guiControls, 'animacion2').name("Péndulo 2 : ").listen();
      animacion.add(this.guiControls, 'velocidad2', 0.0, 2.0, 0.1).name("Velocidad (rad/s) : ").listen();
   ...
   update() {
      ...
      var segundosTranscurridos1 = (tiempoActual - this.tiempoAnterior1) / 1000.0; // Tiempo en segundos
      var segundosTranscurridos2 = (tiempoActual - this.tiempoAnterior2) / 1000.0;

      // Girar péndulo entre -45 y 45 grados
      if (this.pendulo1.rotation.z >= 45*Math.PI/180) {
         this.derecha1 = false;
      } else if (this.pendulo1.rotation.z <= -45*Math.PI/180) {
         this.derecha1 = true;
      }

      if (this.pendulo2.rotation.z >= 45*Math.PI/180) {
         this.derecha2 = false;
      } else if (this.pendulo2.rotation.z <= -45*Math.PI/180) {
         this.derecha2 = true;
      }

      this.parteMovil.scale.y = this.guiControls.h1 / 5.0; // Longitud del péndulo superior
      if (this.guiControls.animacion1) {
         if (this.derecha1 == true) {
            this.pendulo1.rotation.z += this.guiControls.velocidad1 * segundosTranscurridos1;
         } else {
            this.pendulo1.rotation.z -= this.guiControls.velocidad1 * segundosTranscurridos1;
         }
      } else {
      ...
      if (this.guiControls.animacion2) {
         if (this.derecha2 == true) {
            this.pendulo2.rotation.z += this.guiControls.velocidad2 * segundosTranscurridos2;
         } else {
            this.pendulo2.rotation.z -= this.guiControls.velocidad2 * segundosTranscurridos2;
         }
      } else {
      ...
      this.tiempoAnterior1 = tiempoActual;
      this.tiempoAnterior2 = tiempoActual;
   }
}
